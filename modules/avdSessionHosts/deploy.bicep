...
@sys.description('Name of the VM/ Session Host.')
param vmName string 
...
// Session hosts
module sessionHosts '../../avm/res/virtual-machine/main.bicep' =  {
    scope: resourceGroup('${subscriptionId}','${computeObjectsRgName}')
    name: 'SH-module-${vmName}-${time}'
    params: {
        name: vmName
        location: location
        timeZone: timeZone
        zone: useAvailabilityZones ? 1 : 0
        managedIdentities: (identityServiceProvider == 'EntraID' || deployMonitoring) ? {
            systemAssigned: true
        }: null
        encryptionAtHost: encryptionAtHost
        virtualMachineScaleSetResourceId: '/subscriptions/${subscriptionId}/resourceGroups/${computeObjectsRgName}/providers/Microsoft.Compute/virtualMachineScaleSets/${vmssFlexName}'
        osType: 'Windows'
...
module sessionHostsAntimalwareExtension '../../avm/res/virtual-machine/extension/main.bicep' = {
    scope: resourceGroup('${subscriptionId}', '${computeObjectsRgName}')
    name: 'SH-Antimal-${vmName}-${time}'
    params: {
        location: location
...
module monitoring '../../avm/res/virtual-machine/extension/main.bicep' =  if (deployMonitoring) {
    scope: resourceGroup('${subscriptionId}', '${computeObjectsRgName}')
    name: 'SH-Mon-${vmName}-${time}'
    params: {
        location: location
        virtualMachineName: vmName
